...
{
    public class AlmacenadorArchivos : IAlamacenadorArchivos
    {
        ...
        public async Task<string> GuardarArchivo(byte[] contenido, string extension, string contenedor, string contenType)
        {
            ...
            var urlActuall = $"{httpContextAccessor.HttpContext.Request.Scheme}://{httpContextAccessor.HttpContext.Request.Host}";
            var urlParaBD = Path.Combine(urlActuall, contenedor, nombreArchivo).Replace("\\", "/");
            return urlParaBD;
        }
    }
}
